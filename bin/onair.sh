...
# Fichier start radio - Jean-Paul Le Fèvre march 2020
...
function start {
    ...
    if ping_station $station; then
        ...
        url="file://$RADIOG_HOME/lib/buffalo-soldier.mp3"
        echo "Playing $url"
    fi

    $cmd $opt $url 1>$RADIOG_HOME/run/mp.log 2>>$log &
    ...
    if [ -n "$time" ]; then
        ...
        # Clean up the queue
        $RADIOG_HOME/bin/atrmall.sh

        echo "Playing onair.sh for $time minutes ..." >>$log
        at now +$time minutes -f $RADIOG_HOME/bin/offair.sh >>$log
    else
...
if [ -z "$RADIOG_HOME" ]; then
    ...
    echo "Set RADIOG_HOME=$RADIOG_HOME"
fi
